...
- (int)layerIndexWithName:(NSString*)aLayerName {
    ...
    for(Layer *layer in layers) {
        if([[layer layerName] isEqualToString:aLayerName]) {
            NSLog(@"Layer %@ Found in TiledMap.m", aLayerName);
...
- (void)parseMapObjects:(TBXML*)aTmxXML {
    ...
    while (objectGroup) {
        ...
        while (object) {
            ...
            NSLog(@"INFO - Layer.m: ----> Object found called '%@', type:'%@', x:'%@', y:'%@', width: '%@', height: '%@'", objectName, objectType, objectX, objectY, objectWidth, objectHeight);
            ...
            if (properties) {
                ...
                while (property) {
                    ...
                    NSLog(@"INFO - Layer.m: ------> Object property found called '%@', value:'%@'", objectPropertyName, objectPropertyValue);
